...
# ─── Safe defaults for StableGen scene properties ───────────────────────
# When running without StableGen installed, these properties won't exist
# on the scene. This dict maps property names to their safe defaults.
_SG_SCENE_DEFAULTS = {
    # Projection config
    'generation_method':             'direct',
    'generation_mode':               'generate',
    'model_architecture':            'qwen_image_edit',
    'qwen_generation_method':        'direct',
    ...
    'bake_visibility_weights':       False,
    # Vignette / feathering
    'visibility_vignette':           False,
    'visibility_vignette_width':     0.15,
    'visibility_vignette_softness':  1.0,
    # Refinement ramps
    'refine_angle_ramp_active':      False,
    'refine_angle_ramp_pos_0':       0.0,
    'refine_angle_ramp_pos_1':       0.0,
    'refine_feather_ramp_pos_0':     0.0,
    'refine_feather_ramp_pos_1':     0.0,
    'refine_edge_feather_projection': False,
    # Voronoi / IPAdapter (never active in QwenBuild context)
    'qwen_voronoi_mode':             False,
    'qwen_guidance_fallback_color':  (1.0, 0.0, 1.0),
    'sequential_ipadapter':          False,
    'sequential_ipadapter_regenerate': False,
    'sequential_ipadapter_mode':     'first',
    'use_ipadapter':                 False,
    # Timestamp (used by utils.get_generation_dirs)
    'output_timestamp':              '',
}


class _SceneProxy:
    """Wraps a bpy.types.Scene and provides safe defaults for StableGen
    properties that may not be registered when StableGen is not installed.
    Attribute reads fall through to the real scene first; only if that
    raises AttributeError do we return the corresponding default."""
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            if name in _SG_SCENE_DEFAULTS:
                return _SG_SCENE_DEFAULTS[name]
            raise
    ...
    def __setattr__(self, name, value):
        ...
        try:
            ...
            # Property not registered — store as custom ID property
            try:
                scene[name] = value
            except Exception:
                pass
    ...
    def __getitem__(self, key):
        return object.__getattribute__(self, '_scene')[key]

    def __setitem__(self, key, value):
        object.__getattribute__(self, '_scene')[key] = value

    def __contains__(self, key):
        return key in object.__getattribute__(self, '_scene')


class _ContextProxy:
    """Wraps the Blender context, replacing .scene with a _SceneProxy."""

    __slots__ = ('_ctx', '_scene_proxy')

    def __init__(self, ctx):
        object.__setattr__(self, '_ctx', ctx)
        object.__setattr__(self, '_scene_proxy', _SceneProxy(ctx.scene))
...
def project_image(context, to_project, mat_id, stop_index=1000000):
    ...
    """
    # Wrap context so missing StableGen properties return safe defaults
    ...
    for obj in to_project:
        ...
        for i, camera in enumerate(cameras):
            ...
            if i <= stop_index:
                ...
                # Provide a generic 1x1 black placeholder to prevent Cycles from
                # throwing "Image file does not exist" errors when evaluating the graph.
                tex_image.image = _get_voronoi_placeholder((0,0,0))
                tex_image.interpolation = 'Closest'
